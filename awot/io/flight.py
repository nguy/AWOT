...
def read_nasa_ames(filename, mapping_dict=None, platform=None):
    ...
    # Get the date
    DateStr = str(hdr['DATE'][0])+'-'+str(hdr['DATE'][1])+'-'+str(hdr['DATE'][2])
    StartTime = datetime(hdr['DATE'][0], hdr['DATE'][1], hdr['DATE'][2], 0, 0, 0)
    TimeSec = np.array(readfile['time'][:]) + date2num(StartTime, units=_get_time_units())
