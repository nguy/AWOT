      SUBROUTINE W_DOWN(Z1,TV1,Z2,TV2,Z3,RHO3,
     $N,Z,DIV,
     $I_BC,W_BC,
     $BADDATA,
     $W,
...
C  velocity boundary condition by downward integration of the mass
...
C  Calculate the vertical mass flux below the boundary.
      SUCCESS=.FALSE.
      IF(I_BC.GT.1)THEN
         DO I=I_BC-1,1,-1
     ...
     $      DIV(I).EQ.BADDATA)THEN
            ...
            ENDIF
            RHO_W(I)=RHO_W(I+1)+
     $      DIV(I+1)*RHO_BAR_BELOW(I+1)*DEL_Z_BELOW(I+1)+
     $      DIV(I)*RHO_BAR_ABOVE(I)*DEL_Z_ABOVE(I)
            SUCCESS=.TRUE.
         ENDDO
      ENDIF
