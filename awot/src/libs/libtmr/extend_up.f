      SUBROUTINE EXTEND_UP(N_IN,Z_IN,A_IN,
     $Z_EXTEND,
     $BADDATA,
     $N_OUT,Z_OUT,A_OUT)
...
C  This subroutine extends data up to a specified altitude if the data
...
C  If the datum at the level to extend to is missing, search downward
C  for the next datum.
      IF(A_OUT(IZ_EXTEND).EQ.BADDATA)THEN
         IF(IZ_EXTEND.GT.1)THEN
            DO IZ=IZ_EXTEND-1,1,-1
               IF(A_OUT(IZ).NE.BADDATA)THEN
                  DO JZ=IZ_EXTEND,IZ+1,-1
                     A_OUT(JZ)=A_OUT(IZ)
                  ENDDO
                  RETURN
               ENDIF
