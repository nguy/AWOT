      SUBROUTINE LOAD_INTEGER(K,OUTSTRING,NCHAR)

C  Thomas Matejka NOAA/NSSL 26 March 1993

C  This subroutine writes the integer K left justified into the string
C  OUTSTRING.  If OUTSTRING is longer than needed, the end of OUTSTRING
C  is filled with blanks.
...
C  NCHAR.
      ...
      IF(STORE(1:1).NE.' ')THEN
         WRITE(TMMLIB_MESSAGE_UNIT,*)'LOAD_INTEGER:  MEMORY ',
