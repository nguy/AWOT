      SUBROUTINE HOLE_FILL_EXTEND_2D_ONE_PASS(A_IN,SD_A_IN,CANDIDATE,
     $MAXX_IN,MAXY_IN,DELX,DELY,NX,NY,BADDATA,
...
C  two-dimensional, zeroth-order, weighted linear regression.  Holes in
...
C  points that were filled.
     ...
     $IPOINT,EFFECTIVE_TOTAL_POINTS,NX,NY
      ...
      REAL DELX,DELY,X,Y,SSQ,BADDATA,SUM,COEFF,VAR_COEFF,REG_A_INV,dum
      REAL RESPONSE_VECTOR(NX*NY),VAR_RESPONSE_VECTOR(NX*NY),
     $PREDICTOR_ARRAY(NX*NY)
...
C  Perform the regression.
                     CALL LLS_VAR(1,TOTAL_POINTS,PREDICTOR_ARRAY,
     $               NX*NY,RESPONSE_VECTOR,
     $               VAR_RESPONSE_VECTOR,.FALSE.,dum,.FALSE.,1.,.TRUE.,
     $               HOLE_FILL_SINGULAR_THRESHOLD,0,1,
