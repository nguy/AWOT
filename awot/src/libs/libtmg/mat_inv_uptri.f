      SUBROUTINE MAT_INV_UPTRI(A,A_DIM_1,N,A_INV,A_INV_DIM_1,
     $CHECK_SINGULAR,SINGULAR_THRESHOLD,SUCCESS)

C  Thomas Matejka NOAA/NSSL 13 September 1995

C  This subroutine calculates the inverse of a square, upper-triangular
C  matrix.

C  Input:

C  A is a two-dimensional real array.  A(I,J) specifies the element in
C  the Ith row and Jth column of the square, upper-triangular matrix
...
C  Set the elements below the main diagonal to zero.
         IF(J.LT.N)THEN
            DO 2 I=N,J+1,-1
               A_INV(I,J)=0.
2           CONTINUE
...
C  Calculate the elements above the main diagonal.
         IF(J.GT.1)THEN
            DO 3 I=J-1,1,-1
               SUM=0.
               DO 4 K=I+1,J
