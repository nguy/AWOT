...
C  to the third dimension of a three-dimensional data field.
...
C  D2DZ2_A is a three-dimensional real array.  D2DZ2_A(I,J,K) returns
...
C  third dimension.  If it is missing, it is returned as BADDATA.
     ...
     $NZ,IX,IY,IZ
      ...
      REAL D2DZ2_A(MAXX_OUT,MAXY_OUT,MAXZ_OUT)
     ...
     $         A(IX,IY,IZ+1).NE.BADDATA)THEN
                  D2DZ2_A(IX,IY,IZ)=
     $            (A(IX,IY,IZ-1)+A(IX,IY,IZ+1)-2.*A(IX,IY,IZ))/DELZ**2
               ELSE
                  D2DZ2_A(IX,IY,IZ)=BADDATA
               ENDIF
            ENDDO
         ENDDO
      ENDDO
