      SUBROUTINE LLS_PAR_COV(NPARAMS,NDATA,X,X_DIM_1,Y,VAR_Y,
     $CORRELATION,GROUPED,GROUP_SIZE_IN,ESTIMATE_SSQ,SSQ_IN,
     $CHECK_SINGULAR,SINGULAR_THRESHOLD,WRITE_MODE,A_INV_DIM_1,
     $EFFECTIVE_NDATA,C,VAR_C,SSQ,A_INV,SUCCESS)
...
C  a constant correlation between all pairs of response variable data is
C  allowed.  To correct for the variable variances and for the
C  correlations, the regression is performed with transformed data.
C  Data points may be grouped so that each group affects the regression
C  as a single point.  The variance of the response variable data about
C  the regression may be either known a priori or estimated from the
...
C  VAR_Y is a one-dimensional real array.  VAR_Y(I) specifies a value
C  proportional to the response variable variance about a potential
C  regression for the Ith data point.

C  CORRELATION is a real variable that specifies the correlation between
C  every pair of response variable data.  CORRELATION must be greater
C  than -1. and less than 1..
...
C  the values in VAR_VECTOR, gives the variances of the response
...
C  regression could be performed.
     ...
     $X_DIM_1,WRITE_MODE,A_INV_DIM_1,JDATUM
      INTEGER GROUP_SIZE_IN(NDATA)
      INTEGER GROUP_SIZE(NDATA)
      REAL SSQ,SSQ_IN,SSTOT,SSREG,SSRES,SUM,CORRELATION,
     $SINGULAR_THRESHOLD
      REAL C(NPARAMS),VAR_C(NPARAMS)
      REAL Y(NDATA),VAR_Y(NDATA)
      REAL B(NPARAMS)
...
C  Check the memory size.
      IF(A_INV_DIM_1.LT.NPARAMS)THEN
         WRITE(TMMLIB_MESSAGE_UNIT,*)'LLS_PAR_COV:  A_INV_DIM_1 MUST ',
     $   'BE GREATER THAN OR EQUAL TO NPARAMS.'
         STOP
      ENDIF
...
C  Create a response variable variance-covariance matrix to reflect the
C  specified variances and correlation of the response variable data and
C  the grouping of the data.
      DO 3 IDATUM=1,NDATA
         COV_Y_GROUPED(IDATUM,IDATUM)=VAR_Y(IDATUM)*
     $   FLOAT(GROUP_SIZE(IDATUM))
3     CONTINUE
      IF(NDATA.GE.2)THEN
         DO 4 IDATUM=2,NDATA
            DO 5 JDATUM=1,IDATUM-1
               COV_Y_GROUPED(IDATUM,JDATUM)=CORRELATION*
     $         SQRT(VAR_Y(IDATUM)*VAR_Y(JDATUM)*
     $         FLOAT(GROUP_SIZE(IDATUM))*FLOAT(GROUP_SIZE(JDATUM)))
