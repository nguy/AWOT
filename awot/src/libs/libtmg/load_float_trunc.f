      SUBROUTINE LOAD_FLOAT_TRUNC(A,NDECPT_MAX,OUTSTRING,NCHAR)
...
C  If OUTSTRING is longer than needed, the end of OUTSTRING is filled
C  with blanks.
...
C  NCHAR.
      ...
      IF(STORE(1:1).NE.' ')THEN
         WRITE(TMMLIB_MESSAGE_UNIT,*)'LOAD_FLOAT_TRUNC:  MEMORY ',
...
4     CONTINUE
      ...
      IF(NON_ZERO)THEN
         CALL LOAD_STRING(STORE(IMIN:IMAX),OUTSTRING)
         NCHAR=IMAX-IMIN+1
      ELSE
         CALL LOAD_STRING('0',OUTSTRING)
         NCHAR=1
      ENDIF
      RETURN
      END
