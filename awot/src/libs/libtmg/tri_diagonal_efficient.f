      SUBROUTINE TRI_DIAGONAL_EFFICIENT(AP,AP_DIM_1,N,B,SOLUTION,
     $SUCCESS)

C  Thomas Matejka NOAA/NSSL 14 February 2002

C  This subroutine solves a set of simultaneous tri-diagonal linear
C  equations.  The diagonal coefficients are stored efficiently.

C  Input:

C  AP (2D real array 1:AP_DIM_1,-1:1). AP(I,J) specifies the coefficient
C  for the (I+J)th variable in the Ith equation. AP(1,0) must not be
C  zero.
...
C  SOLUTION (1D real array 1:N).  SOLUTION(I) returns the solution for
C  the Ith variable.
...
C  SUCCESS (logical) returns .TRUE. if and only if a solution was found.
      ...
      REAL::TERM
      REAL,DIMENSION(N-1)::A_RR
      REAL,DIMENSION(N)::B,SOLUTION,B_RR
      REAL,DIMENSION(AP_DIM_1,-1:1)::AP
...
C  Calculate the solution.
      ...
      END SUBROUTINE TRI_DIAGONAL_EFFICIENT
