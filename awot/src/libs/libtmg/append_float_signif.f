      SUBROUTINE APPEND_FLOAT_SIGNIF(NBLANKS,A,NSIG,OUTSTRING)
...
C  non-blank character.  The number is written with NSIG significant
C  digits.  The rest of OUTSTRING is filled with blanks.

      IMPLICIT NONE
      INCLUDE 'tmmlib.inc'
      CHARACTER*(MAX_NUMBER_STRING) STORE
      CHARACTER*(*) OUTSTRING
      INTEGER NBLANKS,NSIG,NCHAR
      REAL A

      CALL LOAD_FLOAT_SIGNIF(A,NSIG,STORE,NCHAR)
