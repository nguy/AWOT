      SUBROUTINE LLS_VAR(NPARAMS,NDATA,X,X_DIM_1,Y,VAR_Y,GROUPED,
     $GROUP_SIZE_IN,ESTIMATE_SSQ,SSQ_IN,CHECK_SINGULAR,
...
C  allowed to have different variances about a potential regression but
C  are all assumed to be uncorrelated.  To correct for the variable
C  variances, the regression is performed with transformed data.  Data
...
C  regression for the Ith data point.  VAR_Y(I) must be greater than 0..
...
C  the values in VAR_Y, gives the variances of the response variable
C  data about a potential regression.
...
C  Check the memory size.
      IF(A_INV_DIM_1.LT.NPARAMS)THEN
         WRITE(TMMLIB_MESSAGE_UNIT,*)'LLS_VAR:  A_INV_DIM_1 MUST BE ',
...
C  predictor variable data and the transformed response variable data.
     ...
     $      (VAR_Y(IDATUM)*FLOAT(GROUP_SIZE(IDATUM)))
9        CONTINUE
8     CONTINUE
