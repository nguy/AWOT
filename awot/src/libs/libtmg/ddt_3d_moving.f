      SUBROUTINE DDT_3D_MOVING(A1,A2,A3,MAXX_IN,MAXY_IN,MAXZ_IN,XMIN1,
...
C  This subroutine calculates the partial derivative with respect to
C  time of a three-dimensional data field input at three times.  The
C  data spacing in time does not have to be uniform.  A three-point
C  first-order linear regression is used where possible.  Otherwise, a
C  two-point difference over two time intervals or over one time
C  interval is used where possible.  Time derivatives may be calculated
C  with respect to a moving frame of reference.
...
C  the first time of the data field whose derivative is sought.  If it
...
C  T2 is a real variable that specifies the time for A2 and DDT_A2.
...
C  dimension, and the second time, of the partial derivative of A2 with
C  respect to time in the moving frame of reference.  If it is missing,
