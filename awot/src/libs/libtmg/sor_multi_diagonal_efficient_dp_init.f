      SUBROUTINE SOR_MULTI_DIAGONAL_EFFICIENT_DP_INIT(RELAX,THRESH,
     $MAX_ITERS,N_DIAGS,AP,AP_DIM1,N,B,X_INIT,X,MAX_CHANGE,ITER,SUCCESS)
...
C  efficiently.  Double precision arithmetic is used.  An ititial guess
C  for the solution is input.
...
C  of AP is indexed from -N_DIAGS/2 to N_DIAGS/2.  AP(I,J) specifies the
...
C  X_INIT is a one-dimensional double precision array.  X_INIT(I)
C  specifies the initial guess for the Ith variable.

C  Output:

C  X is a one-dimensional double precision array.  X(I) returns the
C  solution for the Ith variable only if SUCCESS is .TRUE..
...
C  a solution was found within the specified convergence criteria.
      ...
      REAL MAX_CHANGE,RELAX,THRESH
      DOUBLEPRECISION SUM,X_OLD
      DOUBLEPRECISION B(N),X(N),X_INIT(N)
      DOUBLEPRECISION AP(AP_DIM1,-N_DIAGS/2:N_DIAGS/2)

C  Check that there are no zeros on the main diagonal.
      DO I=1,N
         IF(AP(I,0).EQ.0.D0)THEN
            SUCCESS=.FALSE.
            RETURN
         ENDIF
      ENDDO
...
C  Iterate to a solution.
      ...
      DOWHILE(ITER.LT.MAX_ITERS)
         ...
         DO I=1,N
            ...
            ENDDO
            X_OLD=X(I)
            X(I)=X_OLD+DBLE(RELAX)*(B(I)-SUM)/AP(I,0)
            MAX_CHANGE=AMAX1(MAX_CHANGE,SNGL(DABS(X(I)-X_OLD)))
         ENDDO
