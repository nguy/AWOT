Program Fast_Interp_SO_out
  ...
  Write (LM,'("       Starting fast_interp_so_out ",a)') Time_Current
  ...
  If (Line(1:nch) .ne. 'fast_interp_so_out.prm') Then
     Write (LM,'(/"  Improper type of .prm file - must start with fast_interp_so_out.prm ",a/)') Line(1:nch)
     Stop 'fast_interp_so_out'
  End If
...
32 Format (/'.cor file headers:'/ &
  ...
  ! NEW ASCII OUTPUT
  !=============================================================
  open (28,file=Namof(1:nch) //'_so.txt',action='write')
...
End Program Fast_Interp_SO_out
...
Subroutine Cint (Itype, Data)
  ...
  Dimension Vel1(1024), Vel2(1024), Hed1(15), Hed2(15), Ref1(1024), Ref2(1024)
  Dimension radius1(1024), radius2(1024)
  ...
  Equivalence (Ang1,Hed1(1)), (Plat1,Hed1(2)), (Plon1,Hed1(3)), (Gtln1,Hed1(4)), (Zgat1,Hed1(5)), (Azraw1,Hed1(6)),  &
       ...
       (Ra1,Hed1(13)), (ElHor1,Hed1(14)), (Tsec1,Hed1(15))
  ...
  Equivalence (Ang2,Hed2(1)), (Plat2,Hed2(2)), (Plon2,Hed2(3)), (Gtln2,Hed2(4)), (Zgat2,Hed2(5)), (Azraw2,Hed2(6)),  &
       ...
       (Ra2,Hed2(13)), (ElHor2,Hed2(14)), (Tsec2,Hed2(15))
...
1000 Call GetBeam (Itime1, Hed1, Vel1, Ref1, Ierr)
 ...
 Do i = 1, N_Gates1
    radius1(i) = Rdel1 + Float(i-1)*Gtln1
 End Do
...
10 Format (/1X,3I2.2,' Lat:',F7.3,' Lon:',F8.3,' Xz:',F6.2,' Yz:', F6.2,' Alt:',F5.0,' Tilt:',F5.1/ &
  ...
  ! NEW ASCII OUTPUT
  !=============================================================
  write(28,'(i4,2i2,x,i4)') iy,In,id, N_Gates1
!  write(28,'(8f10.3)')Tdif, Olat1, Olon1, Ra1, Azm1, ElHor1, Float(N_Gates1), (radius1(i!), i=1,N_Gates1),(Vel1(i), i=1,N_gates1)
  write(28,'(8f10.3)')Tsec1, Olat1, Olon1, Ra1, Ang1, Azm1, Float(N_Gates1), (radius1(i), i=1,N_Gates1),(Vel1(i), i=1,N_gates1)
  !=============================================================
...
!  Compute the aircraft's position in the grid
 ...
 Do i = 1, N_Gates2
    radius2(i) = Rdel2 + Float(i-1)*Gtln2
 End Do
...
!====================================================================
! WRITE IN NEW ASCII 
! write(28,'(8f10.3)')Tdif, Olat2, Olon2, Ra2, Azm2, ElHor2, Float(N_Gates2), (radius2(i),i=1,N_Gates2),(Vel2(i), i=1,N_Gates2)
 write(28,'(8f10.3)')Tsec2, Olat2, Olon2, Ra2, Ang2, Azm2, Float(N_Gates2), (radius2(i),i=1,N_Gates2),(Vel2(i), i=1,N_Gates2)
...
Subroutine GetBeam (Itime, Hed, Vel, Ref, Ierr)
  ...
  !  Hed(14) = Elevation angle from horizontal (Nick Guy for SO code)
  !  Hed(15) = Seconds since 0 UTC
...
1 Read (75,End=1000,Err=999,Iostat=Ierr) (Ihead(i),i=1,45), (Iheadr(i),i=1,40)
  ...
  ! Seconds since 0 UTC of current day
  Hed(15) = (Float(Itime(4))*3600.) + (Float(Itime(5))*60.) + Float(Itime(6))
  ...
  Call Correct_Ray (Vel, Ref, Ihead, Tilt, Rot, CompAz, Azm_RC, Elev_Hor)
  ...
  ! Elevation angle of ray from horizontal
  Hed(14) = Elev_Hor
...
Subroutine Correct_Ray (Vel, Ref, Ihead, Tilt_TR, Rot_TR, CompAz, Rot_RC, Elev_Hor)
  ...
  Call Trans_Coords (Rot_RC, Tilt, Dft, Ptc, Trck, CompAz, Rot_TR, Tilt_TR, Elev_Hor)
...
Subroutine Trans_Coords (Rotation, Tilt_Raw, Drft, Ptch, Trck, Azimuth, Elev_Zen, Tilt_TR, Elev_Hor)
...
!  write(*,*)'Elev Angle=',Elev_Hor
!  write(*,*)'Elev Zen=',Elev_Zen
  Return
