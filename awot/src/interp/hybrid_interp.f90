Program hybrid_interp

  !--------------------------------------------------------------
  !     Program to interpolate ground-based radar date to 2-D grid using
  ...
  !  David Jorgensen NOAA/NSSL July 2006
  ...
  Real, Dimension(MaxG) :: VR, DZ, ZT

  Logical OKTime, EOF

  Common /One/ Imax, Jmax
  Common /Two/ Sx, Sy, Olat, Olon, Nmosm, Su, Sv, Xnyq, Alt, dbz_offset, Ivpr
  ...
  Real, Allocatable :: Ref(:,:)        ! (x,y)
  Real, Allocatable :: Hybrid(:,:)     ! (azms,bins)
  Real, Allocatable :: Polar(:,:,:)    ! (azms,bins,2) 1=dbz; 2=elev
  Real, Allocatable :: Topo(:,:)       ! (x,y)

  Integer*2, Allocatable :: Ihd(:)
  ...
  !--------------------------------------------------------------
  !     Name the log file 'hrfyymmddhhmmss.log Strip off the
  ...
  If (Namdf(nchu:) .eq. 'tape') Then
     Logfile(1:3) = 'hrf'             ! prefix "hrf" for hybrid reflectivity
     Logfile(4:) = Namdf(nchf+5:nchu)

     nch = LenTrim(LogFile)
     LogFile = Logfile(1:nch-1) // 'log'
  End If
  ...
  If (Line(1:nch) .ne. 'HYBRID_INTERP') Then
     Write (LM,'(/"  Improper type of .prm file - must start with ", &
          "Hybrid_interp ",a/)') Line(1:nch)
     Call Exit
  End If
  ...
  Write (LM,'(/"Input Variables:"/ &
       ...
       Olat, Olon, Iynd, Ihms, Su, Sv, Alt, dbz_offset, Ivpr, &
  ...
  Call Readuf_File (Azm, Elev, VR, DZ, ZT, MaxG, N_Gates, Iy, In, Id, Ih, Im, Is, Rdel, Gtln, Rlat, Rlon, Ialt, & 
       Isn, Irn, Rname, EOF)
  ...
  Write (95) Ident, Time_Current, Imax, Jmax, Sx, Sy, Rname, &
       ...
       Xnyq, Rlat, Rlon, Itype, Alt_Flag, Alt, dbz_offset, Dum, Dum
  ...
  Write (LM,12) Namof(1:ncho), Time_Current, Imax, Jmax, Sx, &
       ...
       Alt, dbz_offset, Hinf, Xsize, Ysize, Alt_Flag, Ivpr, DelZ1, tanb 
...
12 Format (/'Header for ',a,/ &
       ...
       '  Domain Size:',1x,2F7.2/ &
       '  Alt Flag:',1x,F4.1/ &
       '  IVPR:',i1/ &
       '  DelZ1:',F7.2/ &
       '  tanb:',F7.2)

  Allocate (Polar(Nazms,Nbins,2), Hybrid(Nazms,Nbins))
  ...
  Call Cnvt_Cartesian (Polar, Nazms, Nbins, RdelH, GtlnH, Azm_Step, &
       Ref, Imax, Jmax, Sx, Sy, Olat, Olon, Rlath, Rlonh, Hinf) 

  !  Convert back to dBZ

  Do i = 1, Imax
     Do j = 1, Jmax
        If (Ref(i,j) .ne. -999.0) Ref(i,j) = 10.0 * Alog10(Ref(i,j))
     End Do
  End Do
  ...
  Do j = 1, Jmax
     Do i = 1, Imax
        Ihd(i) = Nint(Ref(i,j) * 10.0) ! =reflectivity (dbZ)
     End Do

     Write (95) (Ihd(i),i=1,Imax) 
  End Do
...
Subroutine Hreflec(Hybrid, Polar, Topo)
  ...
  Parameter (Nelevs = 14)
  Parameter (Max_Gates = 2048)

  Dimension Ref(Max_Gates), Vel(Max_Gates), ZT(Max_Gates)
  Dimension Polar(Nazms,Nbins,2), Hybrid(Nazms,Nbins), Topo(Imax,Jmax), nVCP12(Nelevs)

  Character Rname*8
  Logical*4 TimeOnBefore, EOF
...
1 Call Readuf_File (Azm, Elev, Vel, Ref, ZT, Max_Gates, N_Gates, &
...
Subroutine Readuf_File (Azm, Elev, VR, DZ, ZT, Max_Gates, N_Gates, Iy, In, Id, Ih, Im, Is,  &
  ...
  !          DZ or RE     Reflectivity [dBZ] with ground clutter cancellation
  !          ZT           Raw Reflectivity [dBZ]
  !          SQ           Signal Quality Index [0-1]
  !--------------------------------------------------------------

  Parameter (Lrec = 10000)         ! Nax record size in 16 bit words
  Parameter (MaxF = 6)             ! Number of fields allowed
  Parameter (Lenoh_max = 20, Lenlh_max = 20, Lendh_max = 15)
  Parameter (Lenfh_max = 40)

  Character*8 Rname, SitNam, GenNam
  Character*2 Char2, Ifname(MaxF)

  Logical*4 EOF

  Dimension DZ(Max_Gates), VR(Max_Gates), ZT(Max_Gates)
  Dimension SQ(Max_Gates), SW(Max_Gates)

  Integer Man_Header(45), Iopt_Header(Lenoh_max), Loc_Header(Lenlh_max)
  Integer Ifld_Header(Lenfh_max,MaxF), Idat(Max_Gates,MaxF), Lenfh(MaxF), Idat_Header(Lendh_max)
...
50 Read (50,End=2000,Err=3000,Iostat=Ierr) Char2, Iobuf(2), (Iobuf(k),k=3,Iobuf(2)) 
  nrec = nrec + 1
...
12   Format ('Record length=',I9,' exceeds buffer size=',I5)
  ...
  Do j = 1, Nfe
     Rdel = Float(Ifld_Header(3,j))
     Gtln = Float(Ifld_Header(5,j))/1000.0
     N_Gates = Ifld_Header(6,j)

     If (Ifname(j) .eq. 'ZT') Then    ! the 'ZT' field         
        ...
        !  Re-adjust some values in case first gate(s) discarded because of (-) range

        RdelZ = RdelZ + Float(N_GatesZ - ij) * GtZ ! Readjust RdelR
        N_GatesZ = ij                              ! Readjust N_gatesR
     End If   ! For ZT

     If (Ifname(j) .eq. 'DZ' .or. Ifname(j) .eq. 'RE') Then     !  the 'DZ' or 'RE' (for 88Ds) field
        Scale = Ifld_Header(2,j)
     ...
     If (Ifname(j) .eq. 'VR' .or. Ifname(j) .eq. 'VE') Then      ! the 'VR' field
        ...
        RdelV = RdelV + Float(N_GatesV -ij) * GtV ! Re-adjust RdelV
        N_GatesV = ij                             ! Re-adjust N_gatesV
     End If   ! for VR or VE
  End Do                     ! j=1,Nfe
...
Subroutine Cnvt_Cartesian (Polar, Nazms, Nbins, Rdel, Gtln, Azm_Step, Cart, &
  ...
  Dimension Polar(Nazms,Nbins,2), Cart(Imax,Jmax)
  Dimension Weight(Imax,Jmax)
        ...
        Do i = I_min, I_max
           ...
           Do j = J_min, J_max
              ...
              If (R_sqr .gt. D_sqr) Then
                 ...
                 V = Cart(i,j)
                 Vnew = Polar(Iazm,Igate,1)

                 If (Vnew .ne. -999.0) Then                  
                    V = V * Weight(i,j) + Vnew * W 
                    V = V / (Weight(i,j) + W)
                    Cart(i,j) = V
                    Weight(i,j) = Weight(i,j) + W 
                 End If
              End If
           End Do
        End Do
